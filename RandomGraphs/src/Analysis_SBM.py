...
for f in files:
    ...
    [d,e,g] = scipy.optimize.curve_fit(gcc, data[growth,0], data[growth,7]/N, p0=[data[plateau[-1],0],5,10], bounds = [[data[plateau[-1],0],-np.inf,-np.inf], [data[plateau[-1],0]+1,np.inf,np.inf]] )[0]
    ...
    [a,b,c,z] = scipy.optimize.curve_fit(tot_vertices, data[:,0], data[:,5]/N, p0=[plateau[-1],7,1,10])[0]
    ...
    # Second figure
    plt.figure(2)
    plt.semilogy(data[:,0],data[:,5]/N, 'o', label='N='+str(N), color = colors[j])
    plt.legend()
    plt.xlabel(r'Relative probability $\left[\frac{p}{p_c}\right]$')
...
plt.xlabel("Network size [N]")
plt.ylabel(r'$\delta$')
plt.semilogy(sizes,exp_stretch_gcc,'o', label = r'$\delta$ from (1)')
plt.semilogy(sizes,exp_stretch_totvert,'<', label = r'$\delta$ from (2)')
plt.legend()
plt.show()
...
plt.plot(sizes,shift_totvert,'<', label = r'$\gamma$ from (2)')
plt.legend()
plt.show()
...
plt.xlabel("Network size [N]")
plt.ylabel(r'$\beta$')
plt.plot(sizes,exp_factor_gcc,'o', label = r'$\beta$ from (1)')
plt.plot(sizes,exp_factor_totvert,'<', label = r'$\beta$ from (2)')
plt.legend()
plt.show()

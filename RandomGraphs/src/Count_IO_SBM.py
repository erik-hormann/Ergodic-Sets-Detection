...
# -*- coding: utf-8 -*-
"""
Created on Sat May 31 10:46:03 2021

@author: erikhormann

This program counts the number of inputs/outputs in a synthetic SBM graph
The graphs are created in the SBM(N, P, [N/2, N/2])) form. 
The probability matrix is chosen as
    | 2*p  p |
P = |        | matrix, with p around p_c
    | p  2*p |
    
Many values of the probabilty p around the critical probability p_c are tested
and many samples of SBM graphs are created for each instance of p. The number 
of inputs and outputs are then averaged across the samples for each value of p.
"""
...
for j in range(len(probabilities)):
    ...
    for i in range(samples):
        ...
        X = [int(N/2), int(N/2)]
        P = [[2*p, p], [p, 2*p]]
        G = igraph.Graph.SBM(N, P, X, directed='TRUE')
...
p_bar.close()
savefile = "../data/Count_IO_SBM"+str(N)+".dat"
...
plt.semilogy(results[:,0],results[:,1], label='inputs')
plt.title("Average number of ergodic sets in an SMB model")
plt.xlabel('p/p_c')
plt.semilogy(results[:,0],results[:,3], label='outputs')
...
plt.title("Total number of vertices in ergodic sets [SBM model]")
plt.legend()
plt.xlabel('p/p_c')
plt.show()
...
plt.title("Propotion of vertices in the largest ergodic sets [SBM model]")
plt.legend()
plt.xlabel('p/p_0')
plt.show()